				...
				if (parseInt(jsonData.summary.products.length) == 0)
				{
					...
					else
					{
						$('#center_column').children().each(function() {
							if ($(this).attr('id') !== 'emptyCartWarning' && $(this).attr('id') !== 'emptyCartWarningBack' && $(this).attr('class') !== 'breadcrumb' && $(this).attr('id') !== 'cart_title')
							{
						...
						$('#center_column .cart_summary_back_to_catalog').remove();										 
						$('#emptyCartWarningBack').fadeIn('slow');
					}
				}
				else
				{
	...
	$.ajax({
		...
		success: function(jsonData)
		{
			...
			else
			{
				...
				if(jsonData.summary.delivery_state != undefined)
					updataNpBlock(jsonData.summary.delivery_state);
			}
		},
		error: function(XMLHttpRequest, textStatus, errorThrown) {
			if (textStatus !== 'abort')
...
function updateCartSummary(json)
{
	...
	for (i in product_list)
	{
		...
		{
			current_price_value = product_list[i].price;
		...
		{
			current_price_value = product_list[i].price_wt;
		...
		// Update cart price and old price according to the quantity - Silvito
		var product_quantity = 1;
		if ($('input[name=quantity_' + key_for_blockcart_nocustom + '_hidden]').length > 0) {
			product_quantity = parseInt($('input[name=quantity_' + key_for_blockcart_nocustom + '_hidden]').val());
		} else {
			product_quantity = parseInt($('input[name=quantity_' + key_for_blockcart_nocustom + ']').val());
		}
		current_price_value = current_price_value * product_quantity;
		current_price = formatCurrency(current_price_value, currencyFormat, currencySign, currencyBlank);
		var old_price = formatCurrency(product_list[i].price_without_quantity_discount * product_quantity, currencyFormat, currencySign, currencyBlank);
		$('#product_' + key_for_blockcart_nocustom + ' .order-item-price .price').html(current_price);
		if (product_list[i].price_without_quantity_discount) {
			$('#product_' + key_for_blockcart_nocustom + ' .order-item-price .old-price').html(old_price);
		}
...
function refreshDeliveryOptions()
{
	$.each($('.delivery_option_radio'), function() {
		if ($(this).prop('checked'))
		{
			...
			// Update liqpay payment option - hide for courier delivery option
			var key = $(this).data('key').split(',');
			updateLiqpayPaymentMethod(key)
		}
		else
...
// Update liqpay payment option - hide for courier delivery option
function updateLiqpayPaymentMethod(key)
{
	if (key && key[0] == 26) {
		$('div.row .ecm_liqpay').closest('div.row').hide();
	} else {
		$('div.row .ecm_liqpay').closest('div.row').show();
	}
}
		...
		success : function(request) {
			...
			var idAddress_invoice = ($('#opc_id_address_invoice').length == 1 ? $('#opc_id_address_invoice').val() : ($('#addressesAreEquals:checked').length == 1 ? idAddress_delivery : ($('#id_address_invoice').length == 1 ? $('#id_address_invoice').val() : idAddress_delivery)));
				...
				$.ajax({
					...
					cache: false,
					dataType : "html",
					data: 'allow_refresh=1&ajax=true&method=updateExtraAddressEdit&id_address_delivery=' + idAddress_delivery + '&id_address_invoice=' + idAddress_invoice + '&token=' + static_token + '&np_warehouse=' + $('#np_block option:selected').text(),
					success : function(request) {
